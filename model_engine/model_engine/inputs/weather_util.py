...
def astro_torch(day, latitude: torch.Tensor, radiation: torch.Tensor) -> astro_nt:
    """
    ...
    DEC = -torch.asin(torch.sin(23.45 * RAD) * torch.cos(2.0 * torch.pi * (IDAY + 10.0) / 365.0))
    SC = 1370.0 * (1.0 + 0.033 * torch.cos(2.0 * torch.pi * IDAY / 365.0))
    ...
    DAYL = torch.where(
        torch.abs(AOB) <= 1.0,
        12.0 * (1.0 + 2.0 * torch.asin(AOB) / torch.pi),
        torch.where(AOB > 1.0, 24.0, 0.0),
    )

    DSINB = torch.where(
        torch.abs(AOB) <= 1.0,
        3600.0 * (DAYL * SINLD + 24.0 * COSLD * torch.sqrt(1.0 - AOB**2) / torch.pi),
        3600.0 * (DAYL * SINLD),
    )
    DSINBE = torch.where(
        torch.abs(AOB) <= 1.0,
        ...
        * (
            ...
            + 12.0 * COSLD * (2.0 + 3.0 * 0.4 * SINLD) * torch.sqrt(1.0 - AOB**2) / torch.pi
        ),
    ...
    DAYLP = torch.where(
        torch.abs(AOB_CORR) <= 1.0,
        12.0 * (1.0 + 2.0 * torch.asin(AOB_CORR) / torch.pi),
    ...
    ATMTR = torch.where(DAYL > 0.0, AVRAD / ANGOT, 0.0)
    ...
    FRDIF = torch.where(
        ...
        torch.where(
            ...
            1.33 - 1.46 * ATMTR,
            torch.where((ATMTR <= 0.35) & (ATMTR > 0.07), 1.0 - 2.3 * (ATMTR - 0.07) ** 2, 1.0),
        ),
    )
...
def check_angstromAB(xA: float, xB: float) -> list[float]:
    """
