...
from model_engine.inputs.weather_util import reference_ET, check_angstromAB
...
class NASAWeatherDataContainer(SlotPickleMixin):
    ...
    def to_tensor(self, device: torch.device = "cpu") -> None:
        """
...
class NASAPowerWeatherDataProvider(WeatherDataProvider):
    ...
    def _get_and_process_NASAPower(self, latitude: float, longitude: float) -> None:
        ...
        self.elevation = powerdata["geometry"]["coordinates"][2]
    ...
    def _estimate_AngstAB(self, df_power: pd.DataFrame) -> tuple[float, float]:
        ...
        angstrom_a = np.percentile(relative_radiation[ix].values, 5)
        angstrom_ab = np.percentile(relative_radiation[ix].values, 98)
    ...
    def _query_NASAPower_server(self, latitude: float, longitude: float) -> dict[str, float]:
    ...
    def _get_cache_filename(self, latitude: float, longitude: float) -> str:
        ...
        WEATHER_DATA_USER_HOME = os.path.join(user_path, ".weather_data")
    ...
    def _make_WeatherDataContainers(self, recs: dict[str, float]) -> None:
        ...
        for rec in recs:
            ...
            rec.update({"E0": E0 / 10.0, "ES0": ES0 / 10.0, "ET0": ET0 / 10.0, "IRRIG": 0.05})
    ...
    def _process_POWER_records(self, powerdata: pd.DataFrame) -> pd.DataFrame:
        ...
        fill_value = powerdata["header"]["fill_value"]
    ...
    def _POWER_to_PCSE(self, df_power: pd.DataFrame) -> pd.DataFrame:
